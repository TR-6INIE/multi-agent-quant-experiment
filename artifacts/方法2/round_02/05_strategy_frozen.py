...
class Strategy:
    ...
    def decide(self, context):
        ...
        for lookback in (20, 40, 60):
            ...
            # Downside deviation with minimum acceptable return (MAR) = 0.
            # Only negative daily returns contribute to the risk penalty.
            downside_rets = np.minimum(daily_rets, 0.0)
            downside_vol = np.sqrt(np.nanmean(downside_rets**2, axis=0))

            # Zero downside volatility is treated as the EPS floor, preserving
            # strong low-downside names.  NaN volatility from missing data is
            # kept as NaN so it cannot be silently amplified by the EPS floor.
